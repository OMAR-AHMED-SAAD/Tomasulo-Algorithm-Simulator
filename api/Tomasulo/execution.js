export const executeAddSub = (addSubReservationStation, latencies, issuingTable, cycleCount) => {
  Object.values(addSubReservationStation).forEach((station) => {
    if (station.busy == 1) {
      ...
      if (station.Time == parseInt(latencies[station.op]) && station.Qj == 0 && station.Qk == 0) {
        issuingTable.find((instruction) => instruction.issue == station.issueCycle).startExecution =
          cycleCount;
      }
...
export const executeMultDiv = (multDivReservationStation, latencies, issuingTable, cycleCount) => {
  Object.values(multDivReservationStation).forEach((station) => {
    if (station.busy == 1) {
      ...
      if (station.Time > 1 && station.Qj == 0 && station.Qk == 0) station.Time--;
      else if (station.Time == -1) station.Time = parseInt(latencies[station.op]);
    }
  });
};
...
export const executeLoad = (loadBuffer, latencies, issuingTable, cycleCount) => {
  Object.values(loadBuffer).forEach((buffer) => {
    if (buffer.busy == 1) {
      if (buffer.Time == parseInt(latencies[buffer.op])) {
        issuingTable.find((instruction) => instruction.issue == buffer.issueCycle).startExecution =
          cycleCount;
      }
...
export const executeStore = (storeBuffer, latencies, issuingTable, cycleCount) => {
  Object.values(storeBuffer).forEach((buffer) => {
    if (buffer.busy == 1) {
      if (buffer.Time == parseInt(latencies[buffer.op]) && buffer.Qj == 0) {
        issuingTable.find((instruction) => instruction.issue == buffer.issueCycle).startExecution =
          cycleCount;
      }
      if (buffer.Time > 1 && buffer.Qj == 0) buffer.Time--;
      else if (buffer.Time == -1) buffer.Time = parseInt(latencies[buffer.op]);
    }
  });
};
